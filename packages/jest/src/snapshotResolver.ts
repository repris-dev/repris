...
export const buildSnapshotResolver = async (
  ...
  const resolver =
    cache.get(key) ?? (await createSnapshotResolver(await localRequire, config.snapshotResolver));
...
function createDefaultSnapshotResolver(): SnapshotResolver {
  return {
    ...
    resolveTestPath: (snapshotPath: string) =>
      path.resolve(path.dirname(snapshotPath), '..', path.basename(snapshotPath, DOT_EXTENSION)),
...
async function createCustomSnapshotResolver(
  ...
  const customResolver: SnapshotResolver = {
    resolveSnapshotPath: (testPath: string) => custom.resolveSnapshotPath(testPath, DOT_EXTENSION),
    resolveTestPath: (snapshotPath: string) => custom.resolveTestPath(snapshotPath, DOT_EXTENSION),
