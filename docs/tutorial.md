...
The test and benchmark ran. The report contains a summary of each sample that was collected from each benchmark:

- __Iter.__ - The number of iterations of `fib()`
...
To increase accuracy, multiple samples should be collected. (See [The Statistics of Repris](./statistics-of-repris.md) for more on this.) Running the tests again produces a new sample:
...
Statistics are reported in the same way. You can also see the index summary has updated:
...
Hypothetically if we're happy with the improvement, we can update the baseline again to correspond to the new implementation. Alternatively if we're not happy with the improvement (or if there wasn't one) we can reset the index with `repris reset`:
...
It's common to want to run a benchmark over a series of inputs of different sizes/magnitudes. We can do this with `bench.each()`, which works like Jest's [`test.each()`](https://jestjs.io/docs/api#testeachtablename-fn-timeout). Here we create a parameterized benchmark of `fib(n)`:
...
Now we have 3 benchmarks of `fib(n)` with different values of `n`. From the initial results we can see the algorithm has an approximately linear growth rate:
